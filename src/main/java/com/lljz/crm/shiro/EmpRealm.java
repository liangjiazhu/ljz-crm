...
public class EmpRealm extends AuthorizingRealm {
    ...
    protected AuthenticationInfo doGetAuthenticationInfo(AuthenticationToken authenticationToken) throws AuthenticationException {
        ...
        //拿到前端传过来的用户名
        String eName = (String) authenticationToken.getPrincipal();

        //从数据库中获取用户名密码，用于判断用户信息   【根据用户名，获取user对象】
        Employee emp = service.selectByName(eName);
        ...
        SimpleAuthenticationInfo info = new SimpleAuthenticationInfo(
                emp,                           //用户对象
                emp.getPass(),             //用户密码
                ByteSource.Util.bytes(emp.getSalt()),  //使用 强化盐 = userName + salt
                getName());                             //Realm的名称
        return info;
    }
